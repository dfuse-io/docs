 ...
 *
 * Requierements:
 *  - Have an environment variable named DFUSE_API_TOKEN containing your dfuse API token
...
function readConfig() {
  const endpoint = process.env.DFUSE_API_URL || "https://jungle.eos.dfuse.io"
  ...
  const dfuseApiToken = process.env.DFUSE_API_TOKEN
  if (dfuseApiToken === undefined) {
    console.log(
      "You must have a 'process.env.DFUSE_API_TOKEN' environment variable containing your dfuse API token."
    )
    process.exit(1)
  }
