...
* `web`: adapts rate limits _JWT issuance_ to normal growth, eventually manageable by yourself; enforces `Origin`; each JWT is limited to a tiny amount of IPs; heavily rate l
imits the number of calls per IP.
