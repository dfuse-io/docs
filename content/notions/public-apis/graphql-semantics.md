...
* `query:"receiver:eosio.token account:eosio.token action:transfer"` - The query utilizing the {{< external-link title="dfuse Search Query Language" href="/notions/public-apis/search-query-language" >}} that you would like responses to match. This query requests responses for `transfer` actions on the `eosio.token` smart contract.
